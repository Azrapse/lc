...
*/
	...
	{	
		...
		function head_script() // change style of WYSIWYG editor to match theme better
		{
			...
			$this->output('<script src="/oraculo/qa-content/customcode.js?19" type="text/javascript"></script>');
			$this->output('<link rel="stylesheet" type="text/css" href="/oraculo/qa-theme/Snow/smoothness/jquery-ui-1.9.2.custom.min.css" />');
			$this->output('<link rel="stylesheet" type="text/css" href="/oraculo/qa-content/customstyle.css" />');
		...
		function nav_user_search() // outputs login form if user not logged in
		{
			if (!qa_is_logged_in()) {
				...
				if (isset($login)) {
					...
					$this->output('¿Ya estás dado de alta?', '<a href="'.$login['url'].'" id="oraculoLogin">'.$login['label'].'</a>');
