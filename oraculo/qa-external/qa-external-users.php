...
*/
	...
	//	Until you edit this function, don't show login, register or logout links
		...
		return array(
			'login' => $relative_url_prefix.'../users/login?redirect='.urlencode('oraculo/'.$redirect_back_to_url),
			'register' => $relative_url_prefix.'../users/create_account?redirect='.urlencode('oraculo/'.$redirect_back_to_url),
			'logout' => $relative_url_prefix.'../users/logout?redirect='.urlencode('oraculo/'.$redirect_back_to_url)
		);
...
*/
	...
	//	Until you edit this function, nobody is ever logged in
		...
		// Load the file that bring the CakePHP session
		require("../app/webroot/cake_session.php");
		//print_r($_SESSION['Auth']); 
		$userId = null;
		if(isset($_SESSION['Auth']) and isset($_SESSION['Auth']['User'])){
			$userId = $_SESSION['Auth']['User']['id'];
			$userLogin = $_SESSION['Auth']['User']['username'];
			$userName = $_SESSION['Auth']['User']['fullname'];
			$userRoleId = $_SESSION['Auth']['User']['role_id'];
		}		
		// Allow only ADMIN or LAWYER users
		if ($userId and ($userRoleId == 1 or $userRoleId == 5)) {
			$userid=$userId;
			...
			if (is_array($result))
				return array(
					...
					'email' => $result['email'],
					'userlogin' => $userLogin,
					'level' => ($userLogin=='admin') ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC
				);
		}
...
*/
	...
	//	Until you edit this function, always return null
		...
		if (count($publicusernames)) {
			...
			$results=mysql_query(
				'SELECT fullname, id FROM users WHERE fullname IN ('.implode(',', $escapedusernames).')',
				$qa_db_connection
			);
	
			while ($result=mysql_fetch_assoc($results))
				$publictouserid[$result['fullname']]=$result['id'];
		}
...
*/
	...
	//	Until you edit this function, always return null
		...
		if (count($userids)) {
			...
			$results=mysql_query(
				'SELECT fullname, id FROM users WHERE id IN ('.implode(',', $escapeduserids).')',
				$qa_db_connection
			);
	
			while ($result=mysql_fetch_assoc($results))
				$useridtopublic[$result['id']]=$result['fullname'];
		}
